...
// 16-bit counter, chain with 3 channels, count us for up to 65 seconds
...
void my_isr() {
  TMRx->CH[CHn].CSCTRL &= ~(TMR_CSCTRL_TCF1);  // clear
  ticks++;
  isrms = millis();
...
void isr_init(int hz) {
  ...
  TMRx->CH[CHn].CTRL = 0; // stop
  TMRx->CH[CHn].LOAD = 0;  // start val after compare
  TMRx->CH[CHn].COMP1 = cnt - 1;  // count up to this val, interrupt,  and start again
  TMRx->CH[CHn].CMPLD1 = cnt - 1;
  TMRx->CH[CHn].CTRL = TMR_CTRL_CM(1) | TMR_CTRL_PCS(8 + pcs) | TMR_CTRL_LENGTH ;  // prescale
  attachInterruptVector(IRQ_QTIMERx, my_isr);
  TMRx->CH[CHn].CSCTRL &= ~(TMR_CSCTRL_TCF1);  // clear
  TMRx->CH[CHn].CSCTRL |= TMR_CSCTRL_TCF1EN;  // enable interrupt
  NVIC_ENABLE_IRQ(IRQ_QTIMERx);
}

void cascade() {
  // CH0 tick every 1 ms, clock 150mhz/128  1171875 hz, comp 1172-1 ish
  ...
  TMRx->CH[CHn].COMP1 = cnt - 1;  // count up to this val, interrupt,  and start again
  TMRx->CH[CHn].CMPLD1 = cnt - 1;
  TMRx->CH[CHn].CTRL = TMR_CTRL_CM(7) | TMR_CTRL_PCS(4) | TMR_CTRL_LENGTH ;  //clock from clock 0
  attachInterruptVector(IRQ_QTIMERx, my_isr);
  TMRx->CH[CHn].CSCTRL &= ~(TMR_CSCTRL_TCF1);  // clear
  TMRx->CH[CHn].CSCTRL |= TMR_CSCTRL_TCF1EN;  // enable interrupt
  NVIC_ENABLE_IRQ(IRQ_QTIMERx);
...
void setup()   {
  ...
  PRREG(TMRx->CH[CHn].CTRL);
  PRREG(TMRx->CH[CHn].SCTRL);
  PRREG(TMRx->CH[CHn].CSCTRL);
  PRREG(TMRx->CH[CHn].LOAD);
  PRREG(TMRx->CH[CHn].COMP1);
  PRREG(TMRx->CH[CHn].CMPLD1);
  PRREG(TMRx->CH[CHn].COMP2);
  PRREG(TMRx->CH[CHn].CMPLD2);
}

void loop()
{
  uint32_t us = TMRx->CH[1].CNTR + 1000 * TMRx->CH[2].HOLD; // could count second chnl 3
  Serial.printf("%d ticks %d ms   %d us\n", ticks, isrms, us);
  delay(1000);
}
